...
_declspec(dllexport) int fn_NetSim_DSR_Metrics(PMETRICSWRITER filename)
{
	...
	//This #ifdef.....#endif block defines the som Clustering Metrics, which shows the packets 
...
#ifdef _SOM_CLUSTERING
	PMETRICSNODE table = init_metrics_node(MetricsNode_Table, "som Clustering Metrics", NULL);
	...
	PMETRICSNODE menu = init_metrics_node(MetricsNode_Menu, "CUSTOM_WSN_METRICS", NULL);
...
{
	fprintf(stderr,"Press Enter to close MATLAB\n");
	_getch();
