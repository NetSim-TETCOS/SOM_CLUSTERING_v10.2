	...
	case PHYSICAL_OUT_EVENT:
		{
			...
			double dDataRate;
			bool txFlag=false;
			//Updating som Clustering/Data Metric
	...
	case PHYSICAL_IN_EVENT: 
		{
			...
			else
			{
				...
				//Add mac in event
				fnpAddEvent(pstruEventDetails);
				//Update som Clustering/Data metrics
...
_declspec(dllexport) int fn_NetSim_Zigbee_Metrics(PMETRICSWRITER metricsWriter)
{
	...
	{
		add_table_row_formatted(false, table, "%d\t%d\t%d\t%f,", NETWORK->ppstruDeviceList[i]->nDeviceId, Data_Packet_Sent_PL[NETWORK->ppstruDeviceList[i]->nDeviceId - 1], Data_Packet_Received_PL[NETWORK->ppstruDeviceList[i]->nDeviceId - 1], Lifetime[NETWORK->ppstruDeviceList[i]->nDeviceId - 1] / 1000000);
	}
